...
1 			整个bean的生命周期，从创建到使用到销毁的过程全部都是由容器来管理（bean的生命周期）
...
3、**beanFactoryPostProcessor**的处理，此处是扩展点PlaceHolderConfigurSupport,ConfigurationClassPostProcessor
...
1，创建出beanfactory，开始循环创建对象，首先会找，如果找不到那么会创建
...
3、调用aware接口相关的方法：invokeAwareMethod(

​	完成BeanName, 获取benaName

​	BeanFactory,  得到工厂，那么可以做很多其他的操作

​	BeanClassLoader  得到类加载器
...
5、调用initmethod方法：invokeInitmethod(),判断是否实现了initializingBean接口，如果有，调用afterPropertiesSet方法，没有就不调用   现在用的话@PostConstruct 
...
​		3、在执行方法调用的时候，会调用生成的字节码文件中，直接回找到DynamicAdvisoredInterceptor类中的intercept方法，从此方法开始执行
...
​		总：spring的事务是由aop来实现的，首先要生成具体的代理对象，然后按照aop的整套流程来执行具体的操作逻辑，正常情况下要通过通知来完成核心功能，但是事务不是通过通知来实现的，而是通过一个**TransactionInterceptor来实现的，然后调用invoke来实现具体的逻辑**
